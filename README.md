...
- **Blame** — right-click any file → *Blame this file* for a window showing every line beside the commit that last touched it. Attribution is written once per run of lines from the same commit, and each commit gets a stable colour from the graph's lane palette. Click a line and the commit list jumps to that commit (fetching it if it sits beyond the loaded page), so the diff panel shows the change without leaving blame. Right-click the gutter for *Blame before &lt;sha&gt;* to walk back through a file's history, with Back to return. Blaming from a commit's file list blames *that* revision, not HEAD.
